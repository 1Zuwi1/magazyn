...
public class KmsSecretCrypto implements AutoCloseable {
  ...
  public EncryptedSecret encryptSecret(String plaintext) {
    ...
    try {
      ...
      throw new EncryptionException("encryptSecret failed: KMS envelope encryption error", e);
    } finally {
      zero(dek);
    }
  }
  ...
  public String decryptSecret(String encryptedDekB64, String ciphertextB64) {
    ...
    try {
      ...
      if (dek == null || dek.length == 0)
        throw new EncryptionException("KMS returned empty plaintext for DEK");
      ...
      throw new EncryptionException("decryptSecret failed: KMS envelope decryption error", e);
    } finally {
      zero(dek);
    }
  }
