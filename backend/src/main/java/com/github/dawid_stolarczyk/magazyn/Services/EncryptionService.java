...
public class EncryptionService {
  ...
  private static byte[] buildHeader(byte[] wrapSalt, byte[] dekWrapIv, byte[] dekWrapTag, byte[] dekWrapCt,
    ...
    if (wrapSalt.length != WRAP_SALT_LEN ||
      ...
      throw new EncryptionException("Invalid header component sizes");
    }
  ...
  private static ParsedHeader parseHeader(byte[] buf) {
    if (buf.length < HEADER_LEN)
      throw new EncryptionException("Truncated header");

    for (int i = 0; i < 4; i++) {
      if (buf[i] != MAGIC[i])
        throw new EncryptionException("Bad magic");
    }
  ...
  private static void requireNonEmpty(String value, String label) {
    if (value == null || value.isBlank()) {
      throw new EncryptionException(label + " is blank");
    }
  }
  ...
  public String encrypt(String plaintext) {
    ...
    try {
      ...
      throw new EncryptionException("encrypt failed (plaintext bytes=" + plaintextBytesLen + ")", e);
    } finally {
      if (wd != null) {
        zero(wd.dek);
      }
  ...
  public String decrypt(String containerB64) {
    ...
    try {
      ...
      if (buf.length < ph.headerBytes + TAG_LEN)
        throw new EncryptionException("Truncated data");
      ...
      throw new EncryptionException("decrypt failed (" + detail + ")", e);
    } finally {
      zero(dek);
    }
  }
  ...
  public Path encryptFile(String filePath) {
    ...
    try {
      if (!Files.isRegularFile(src)) {
        throw new EncryptionException("Source is not a file");
      }
      ...
      throw new EncryptionException("encryptFile failed", e);
    } finally {
      if (wd != null) {
        zero(wd.dek);
      }
  ...
  public InputStream decryptFile(String encryptedPath) {
    if (encryptedPath == null || encryptedPath.isBlank()) {
      throw new EncryptionException("Encrypted path is empty");
    }
    ...
    try {
      if (!Files.isRegularFile(enc))
        throw new EncryptionException("Encrypted file not found: " + enc);
      ...
      if (header.length != HEADER_LEN)
        throw new EncryptionException("Truncated file");
      ...
      throw new EncryptionException("decryptFile failed", e);
    }
  }

  private Path resolveWithinBase(String rawPath, String label) {
    if (rawPath == null || rawPath.isBlank()) {
      throw new EncryptionException(label + " path is empty");
    }
    ...
    try {
      ...
      throw new EncryptionException("Invalid " + label + " path", e);
    }
    ...
    if (!normalized.startsWith(baseDir)) {
      throw new EncryptionException(label + " path escapes root");
    }
    return normalized;
  }
  ...
  private Path resolveExistingFile(String rawPath, String label) {
    ...
    try {
      ...
      throw new EncryptionException("Unable to resolve " + label + " path", e);
    }
  }
  ...
  private void enforceFileSize(Path path, long limitBytes, String label) {
    ...
    try {
      ...
      if (size > limitBytes) {
        throw new EncryptionException(label + " file too large");
      }
    } catch (IOException e) {
      throw new EncryptionException("Failed to read " + label + " file size", e);
    }
  }
  ...
  private WrappedDek generateWrappedDEK() throws Exception {
    ...
    try {
      ...
      if (wrapCtPlusTag.length != DEK_LEN + TAG_LEN)
        throw new EncryptionException("Wrapped DEK length mismatch");
  ...
  private byte[] unwrapDEK(byte[] wrapSalt, byte[] dekWrapIv, byte[] dekWrapTag, byte[] dekWrapCt) throws Exception {
    ...
    try {
      ...
      if (dek.length != DEK_LEN)
        throw new EncryptionException("Unwrapped DEK length mismatch");
      return dek;
    } finally {
      zero(root);
      zero(kek);
