...
function calculateWarehouseStats(warehouses: typeof MOCK_WAREHOUSES): {
  ...
  for (const warehouse of warehouses) {
    for (const rack of warehouse.racks) {
      const rackCapacity = rack.rows * rack.cols
      totalCapacity += rackCapacity
      totalUsed += Math.floor((rack.occupancy / 100) * rackCapacity)
    }
  }

  return { totalWarehouses: warehouses.length, totalCapacity, totalUsed }
}
...
export default function WarehousePage() {
  const { totalWarehouses, totalCapacity, totalUsed } =
    calculateWarehouseStats(MOCK_WAREHOUSES)
