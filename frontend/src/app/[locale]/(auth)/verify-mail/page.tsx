...
function VerifyMailContent() {
  ...
  useEffect(() => {
    if (!token) {
      setState("error")
      setErrorMessage(t("generated.auth.verificationTokenUrl"))
      return
    }
    ...
    const verify = async () => {
      ...
      if (err) {
        ...
        setErrorMessage(
          err.message ||
            t("generated.shared.errorOccurredDuringVerificationAgain")
        )
        return
      }
          ...
          >
            <Link className="block" href="/login">
              <Button className="w-full" size="lg">
                {state === "success"
                  ? t("generated.auth.goLogin")
                  : t("generated.auth.backLogin2")}
              </Button>
            </Link>
          </div>
        )}
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="space-y-1.5">
        <h1 className="font-semibold text-lg">
          {t("generated.auth.emailAddressVerification")}
        </h1>
        <p className="text-muted-foreground text-sm">
          {t("generated.auth.waitEmailAddressBeingVerified")}
        </p>
      </div>
      <div className="flex items-center gap-2 text-muted-foreground text-xs">
        <Spinner className="size-3" />
        <span>{t("generated.auth.processing")}</span>
      </div>
    </div>
  )
}
...
function SuccessState() {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="space-y-1.5">
        <h1 className="font-semibold text-lg">
          {t("generated.auth.emailVerified")}
        </h1>
        <p className="text-muted-foreground text-sm">
          {t("generated.auth.emailAddressBeenSuccessfullyVerified")}
        </p>
      </div>
      <div className="w-full rounded-lg bg-emerald-500/5 p-3 ring-1 ring-emerald-500/20">
        <p className="text-emerald-600 text-xs dark:text-emerald-400">
          {t("generated.auth.verificationCompletedSuccessfullyAccountNow")}
        </p>
      </div>
    </div>
  )
