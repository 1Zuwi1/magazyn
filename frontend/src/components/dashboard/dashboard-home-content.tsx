...
const RACKS_SUMMARY_PAGE_SIZE = 5000
...
const buildWarehouseSummary = (warehouse: ApiWarehouse): WarehouseSummary => {
  ...
  return {
    ...
    used,
    occupancy,
    rackCount: warehouse.racksCount,
  }
}
...
export default function DashboardHomeContent() {
  ...
  } = useRacks({ page: 0, size: RACKS_SUMMARY_PAGE_SIZE })

  const warehouseSummaries = useMemo(
    () => (warehousesData?.content ?? []).map(buildWarehouseSummary),
    [warehousesData?.content]
  )

  const totalCapacity = warehouseSummaries.reduce(
    (acc, summary) => acc + summary.capacity,
    0
  )
  const totalUsed = warehouseSummaries.reduce(
    (acc, summary) => acc + summary.used,
    0
  )
  const totalWarehouses =
    warehousesData?.totalElements ?? warehouseSummaries.length
  const totalRacks =
    racksData?.totalElements ??
    warehouseSummaries.reduce((acc, summary) => acc + summary.rackCount, 0)
                ...
                {recentItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-2 text-xs">
                      ...
                      <Badge
                        variant={
                          item.definition.isDangerous ? "warning" : "secondary"
                        }
                      >
